...
def print_final_result(messages):
    """imprimir the final agent result and cost information"""
    # obter the resultado message (último message)
    result_msg = messages[-1]

    # encontrar the último assistant message com actual content
    para msg in reversed(messages):
        se msg.__class__.__name__ == "AssistantMessage" and msg.content:
            # verificar se isso has texto content (não just tool use)
            para block in msg.content:
                se hasattr(block, "text"):
                    imprimir(f"\n📝 Final Result:\n{block.text}")
                    parar
            parar

    # imprimir cost se available
    se hasattr(result_msg, "total_cost_usd"):
        imprimir(f"\n📊 Cost: ${result_msg.total_cost_usd:.2f}")

    # imprimir duration se available
    se hasattr(result_msg, "duration_ms"):
        imprimir(f"⏱️  Duration: {result_msg.duration_ms / 1000:.2f}s")
            ...
            if msg.content:
                for block in msg.content:
                    if hasattr(block, "text"):
                        ...
                        # Show key parâmetros para certain tools
