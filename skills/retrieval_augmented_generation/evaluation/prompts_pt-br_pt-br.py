...
# inicializar the VectorDB
db = VectorDB("anthropic_docs")
# carregar the Anthropic Documentação
...
def answer_query_base(context):
    ...
    Please remain faithful to the underlying context, and only deviate from isso se you are 100% sure that you know the answer already. 
...
def _rerank_results(query: str, results: List[Dict], k: int = 5) -> List[Dict]:
    # Prepare the summaries com their indices
    summaries = []
    print(len(results))
    ...
    # juntar summaries com newlines
    joined_summaries = "\n".join(summaries)
    
    prompt = f"""
    Query: {query}
    You are about to be given a group of documents, each preceded by its index número in square brackets. Your task is to selecionar the only {k} most relevant documents from the list to help us answer the query.
    ...
    try:
        ...
        # Extract the indices de the resposta
        ...
        for idx in indices_str.split(','):
            try:
                ...
                continuar  # Skip invalid indices
        print(indices_str)
        print(relevant_indices)
        # se we didn't obter suficiente valid indices, fall atrás para the topo k by original order
...
def _retrieve_advanced(query: str, k: int = 3, initial_k: int = 20) -> Tuple[List[Dict], str]:
    ...
    # Step 3: Generate novo context texto de re-ranked results
    new_context = ""
...
# The answer_query_advanced função remains unchanged
