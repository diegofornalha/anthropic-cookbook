...
def rouge_eval(summary, ground_truth, threshold=0.3) -> float:
    ...
    threshold (float): The threshold para the ROUGE score (padrão: 0.3).
    
    Retorna:
    bool: verdadeiro se the average ROUGE score is above the threshold, falso otherwise.
    """
    scorer = rouge_scorer.RougeScorer(['rouge1', 'rouge2', 'rougeL'], use_stemmer=True)
    scores = scorer.score(summary, ground_truth)
    
    # Calculate média ROUGE score
