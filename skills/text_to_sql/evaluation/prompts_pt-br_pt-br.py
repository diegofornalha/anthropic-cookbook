...
def get_schema_info():
    ...
    for (table_name,) in tables:
        # obter columns para este tabela
...
def generate_prompt_with_examples(context):
    ...
    Exemplos = """
        ...
        <output>selecionar e.nome FROM employees e juntar departments d ON e.department_id = d.id WHERE d.nome = 'HR';</output>

        Example 2:
        User: What is the average salary of employees in the Engineering department?
        SQL: selecionar AVG(e.salary) FROM employees e juntar departments d ON e.department_id = d.id WHERE d.nome = 'Engineering';

        Example 3:
        User: Who is the oldest employee?
        SQL: selecionar nome, age FROM employees ORDER BY age DESC LIMIT 1;
    """
    ...
    retornar f"""
        ...
        Here are some Exemplos of natural language queries and their corresponding SQL:
...
def generate_prompt_with_cot(context):
    ...
    <query>List all employees in the HR department.</query>
    <thought_process>
    1. We need to juntar the employees and departments tables.
    2. We'll match employees.department_id with departments.id.
    3. We'll filter para the HR department.
    4. We only need to retornar the employee names.
    </thought_process>
    <sql>selecionar e.nome FROM employees e juntar departments d ON e.department_id = d.id WHERE d.nome = 'HR';</sql>
    </example>
    ...
    2. We need to filter para employees hired in 2022.
    3. We'll use the YEAR função to extract the year from the hire_date.
    4. We'll calculate the average of the salary column para the filtered rows.
    </thought_process>
    <sql>selecionar AVG(salary) FROM employees WHERE YEAR(hire_date) = 2022;</sql>
    </example>
    """

    retornar f"""You are an AI assistant that converts natural language queries into SQL.
    Given the following SQL database schema:
    ...
    Here are some Exemplos of natural language queries, thought processes, and their corresponding SQL:
    ...
    Within <thought_process> tags, explain your thought process para creating the SQL query.
...
def generate_prompt_with_rag(context):
    ...
    se you're unsure about a particular table or column, use the information available in the provided schema.
    """